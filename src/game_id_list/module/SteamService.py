...
class SteamService:
    ...
    def _i_store_service_get_app_list_v1(
        self, apps: dict[int, list[str]], language: str
    ) -> None:
        last_appid = 0
        while True:
            try:
                ...
                if (
                    "have_more_results" not in response
                    or not response["have_more_results"]
                ):
                    print(
                        f"IStoreService/GetAppList/v1: l: {language}, last_appid: {last_appid}, "
                ...
                print(
                    f"IStoreService/GetAppList/v1: Error: {repr(ex)}, sleep {self.request_interval}s"
                )
                time.sleep(self.request_interval)
                break
